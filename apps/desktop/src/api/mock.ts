...
import type { Document, DocumentRevision, ExpectedDocumentState, GitWorkingCopySummary, Phase, PhaseSession, Project, Run, RunEvent, RunState, Stage, Task } from "./types";
...
const gitSummary:GitWorkingCopySummary={is_repository:true,branch:"feature/recovery-evidence",latest_commit:{hash:"7c4a81d56b8f54c7a90123456789abcdef012345",subject:"Implement durable task projection",committed_at:"2026-07-21T14:25:00Z"},staged_diff:"diff --git a/src/projection.ts b/src/projection.ts\nindex 1111111..2222222 100644\n--- a/src/projection.ts\n+++ b/src/projection.ts\n@@ -1 +1 @@\n-writeDraft(task);\n+writeAtomically(task);\n",unstaged_diff:"diff --git a/src/recovery.ts b/src/recovery.ts\nindex 3333333..4444444 100644\n--- a/src/recovery.ts\n+++ b/src/recovery.ts\n@@ -1 +1,2 @@\n recover();\n+verifyExpectedState();\n",untracked_paths:["reviews/manual-evidence.md"],truncated:false};

export const mockApi:CraftelApi={
  listProjects:async()=>[project],listDirectory:async(path)=>path?{path,parent:path==="/Users/demo"?"/Users":"/Users/demo",entries:[]}:{path:"/Users/demo",parent:"/Users",entries:[".config","Applications","blog","CascadeProjects","claude-code","CodePilot","conductor","Craftel"].map(name=>({name,path:`/Users/demo/${name}`,hidden:name.startsWith(".")}))},registerProject:async()=>project,openProject:async()=>project,removeProject:async()=>{},listTasks:async()=>[...tasks],
  gitWorkingCopySummary:async()=>({...gitSummary,latest_commit:gitSummary.latest_commit&&{...gitSummary.latest_commit},untracked_paths:[...gitSummary.untracked_paths]}),
