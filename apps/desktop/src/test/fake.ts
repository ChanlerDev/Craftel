...
export function fakeApi(overrides: Partial<CraftelApi> = {}): CraftelApi {
  return { listProjects: vi.fn().mockResolvedValue([]), selectProjectDirectory: vi.fn().mockResolvedValue(null), registerProject: vi.fn(), openProject: vi.fn(), removeProject: vi.fn(), listTasks: vi.fn().mockResolvedValue([]), createTask: vi.fn(), updateTask: vi.fn(), moveTask: vi.fn(), listDocuments:vi.fn(),documentStatus:vi.fn(),readDocument:vi.fn(),searchDocuments:vi.fn(),writeDocument:vi.fn(),listDocumentRevisions:vi.fn(),restoreDocumentRevision:vi.fn(),startPhaseRun:vi.fn(),stopRun:vi.fn(),followUp:vi.fn(),getSession:vi.fn(),listSessions:vi.fn(),listRuns:vi.fn(),getRun:vi.fn(),listRunEvents:vi.fn(), ...overrides };
}
