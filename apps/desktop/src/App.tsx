...
export function App({ api }: { api: CraftelApi }) {
  ...
  const setProject=(p:Project|null)=>{if(!allowLeave())return;setDirty(false);setTask(null);selectProject(p);};
  const leaveTask=()=>{if(!allowLeave())return;setDirty(false);setTask(null)};
  return <div className="shell"><ProjectSwitcher api={api} selected={project} onSelect={setProject} /><main><UpdateControl service={nativeUpdateService} />
