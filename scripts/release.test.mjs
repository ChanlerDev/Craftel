...
import { assertPatchUpgrade, assertReleaseSequence, nextPatchVersion, requestedPatchVersion } from "./release.mjs";
...
test("computes and accepts only the immediate next patch", () => {
  ...
  assert.equal(requestedPatchVersion("0.1.0", undefined), "0.1.1");
  assert.equal(requestedPatchVersion("0.1.0", ""), "0.1.1");
  assert.equal(requestedPatchVersion("0.1.0", "0.1.1"), "0.1.1");
});
...
test("rejects minor, major, skipped, repeated, downgraded, and prerelease versions", () => {
  for (const version of ["0.2.0", "1.0.0", "0.1.2", "0.1.0", "0.0.9", "0.1.1-beta.1"]) {
    ...
    assert.throws(() => requestedPatchVersion("0.1.0", version), /Patch-only releases require 0\.1\.1/);
  }
});
